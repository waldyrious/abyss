...
router.get('/api/me', jwtoptional, function *(next) {
	if (this.state.user) {
		let response = yield dao.getUser(this.state.user.phonenumber);
...
router.post('/api/me', brute, jwtoptional, function *(next) {
	...
	if (this.state.user != undefined && this.request.body.nickname != undefined) {
		...
		let response = yield dao.updateNickname(this.state.user.phonenumber, this.request.body.nickname)
		let user = yield dao.getUser(self.state.user.phonenumber);
		...
		if (this.request.body.phonenumber) {
			...
			if (_.contains(realCodes, code)) {
				...
				yield dao.addSubscriptionToUser(this.state.user.phonenumber, this.session.endpoint) // necessary?
				yield dao.recordLogin(self.state.user.phonenumber, self.request.ip);
...
router.post('/api/registration/subscription', jwtrequired, function *(next) {
	if (this.request.body.endpoint) {
		...
		let response = yield dao.addSubscriptionToUser(this.state.user.phonenumber, this.session.endpoint);
		this.status = 200;
		this.body = {
			success: true,
			message: 'Subscribed.',
...
router.delete('/api/registration/subscription', jwtrequired, function *(next) {
	...
	if (endpoint) {
		yield dao.removeSubscriptionFromUser(this.state.user.phonenumber, endpoint)
		this.status = 204;
	} else {
		this.status = 400;
		this.body = {
...
router.get('/api/conversations', jwtrequired, function *(next) {
	this.response.body = yield dao.getConversations(this.state.user.phonenumber);
});
...
router.get('/api/file/:id', jwtcookie, function*(next) {
	let message = yield dao.getFileMessage(this.state.user.phonenumber, this.params.id);
