...
router.get('/api/file/:id', jwtcookie, function*(next) {
	var self = this;
	// streamed
	//this.body = dao.getFileStreamed(this.state.user.phonenumber, this.params.id);
	...
	this.set('Content-Disposition', 'attachment; filename="' + encodeURIComponent(file.name) + '"');
