...
router.post('/api/messages', jwtrequired, function *(next) {
	...
	var to = arrayify(this.request.query.to);
...
router.get('/api/messages', jwtrequired, function *(next) {
	var to = cleanTo(this.request.query.to);
...
router.get('/api/file/:id', jwtcookie, function*(next) {
	...
	this.set('Last-Modified', message.date);
	...
	this.body = fs.createReadStream(message.file.savedFilePath);
});
...
router.post('/api/file', jwtrequired, function*(next) {
	...
	if (to.length === 0 || to.length === 1 && to[0] == '') { // todo make this better
		to = [this.state.user.phonenumber];
	}
