...
function updateDao() {
	var self = this;
	if (this.state.user && this.state.user.phonenumber && this.session.endpoint) {
		return dao.addSubscriptionToUser(this.state.user.phonenumber, this.session.endpoint)
...
router.post('/api/registration/subscription', jwtrequired, function *(next) {
	...
	if (this.request.body.endpoint) {
		...
		let response = yield updateDao.bind(this)();
		this.status = 200;
		this.body = {
			success: true,
			message: 'Subscribed.',
