...
router.delete('/api/registration/subscription/:id', jwtrequired, function *(next) {
	var self = this;

	const id = this.params.id;

	if (id) {
		yield dao.removeSubscriptionFromUser(this.state.user.phonenumber, this.session.subscriptionId)
		this.status = 204;
	} else {
		this.status = 400;
		this.body = {
