...
router.get('/api/file/:id', function*(next) {
	var self = this;

	var file = yield dao.getFile(this.params.id);

	this.response.body = file.file;
});
...
router.post('/api/file', jwtrequired, function*(next) {
	...
	while (part = yield parts) {
		...
		.then(function (buffer) {
			return dao.addFile(self.state.user.phonenumber, to, 'filename', buffer);
		})
