...
exports.addFileStreamed = Promise.coroutine(function* (fileInfo, inputStream) {
	...
	} catch (e) {} // ok if directory already exists

	let savedFilePath = path.join(secret.blobs, String(from), fileId);
	console.log(savedFilePath)
	yield new Promise(function (resolve, reject) {
		const outputStream = fs.createWriteStream(savedFilePath);
		inputStream.on('end', function () {	resolve(savedFilePath);	})
	...
	let ins = yield r.table('messages')
	.insert({
		from: from,
		to: to,
		date: new Date(),
...
exports.addSubscriptionToUser = function (ph, endpoint) {
	...
	.then(function (response) {
		...
		.get(ph)
		.update({
			subids: r.row('subids').setInsert(endpoint)
			// .limit(20) // limit number of subscribed devices
		})
	});
};

exports.removeSubscriptionFromUser = function (ph, endpoint) {
	// associate subscription id with phone ph.
	return r.table('users')
	.get(ph)
	.update({
		'subids': r.row('subids').setDifference([endpoint])
	})
};
...
const getUpdates = exports.getUpdates = function (ph) {
	return r.table('messages')
	.filter(r.row('to').contains(ph).or(r.row('from').eq(ph)))
	// .map(function (item) {
	// 	return item.merge({'participants': item('to').append(item('from')).distinct().difference([ph])});
...
const getConversations = exports.getConversations = function (ph) {
	...
	.orderBy(r.desc('reduction'))
	.map(function (item) {
		return item.merge({last: item('reduction')(0)('date')})
	})
	.without('reduction')
	.map(function (item) {
		return {
