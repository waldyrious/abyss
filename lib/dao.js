...
exports.addFileStreamed = function (fileInfo, inputStream) {
	...
	try {
		fs.mkdirSync(path.join(secret.blobs, String(to)));
	} catch (e) {}

	return tmp.tmpNameAsync({
		template: path.join(secret.blobs, String(to)) + '/XXXXXX'
	})
	...
	.then(function (savedFilePath) {
		...
		fileInfo.savedFilePath = savedFilePath;
...
var lax = {'returnChanges': true};

function cleanAssociatedFiles(changeSet) {
	Promise.map(changeSet.changes, function (change) {
		console.log('DELETING ' + change.old_val.from + ' ' + change.old_val.file.savedFilePath);
		return fs.unlinkAsync(change.old_val.file.savedFilePath);
	})
}
